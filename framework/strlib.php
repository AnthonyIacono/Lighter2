...
class StrLib {
    ...
    public static function Guidish($prefix = '') {
        return $prefix . str_replace('-', '', self::Guid());
    }
    ...
    /**
     * @static
     * Get's the pieces of a camel case string in an array
     */
    public static function CamelPieces($camelCaseString) {
        $split_keys = preg_replace('/([A-Z])/', ' $1', $camelCaseString);
        $split_keys_exp = explode(' ', $split_keys);
        $output = array();
        foreach($split_keys_exp as $k) {
            $k = strtolower($k);
            if (trim($k) == '') {
                continue;
            }
    ...
    public static function Slugify($inputStr, $slugSeparator = '_') {
        return implode($slugSeparator, self::CamelPieces($inputStr));
